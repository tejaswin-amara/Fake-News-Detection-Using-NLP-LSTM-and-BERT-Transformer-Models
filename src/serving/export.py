...
def build_package_manifest(
    ...
    manifest = {
        ...
        "native_artifact": {"path": str(native_path).replace("\\", "/"), "sha256": sha256_file(native_path) if Path(native_path).exists() else None},
        "onnx_artifact": {"path": str(onnx_path).replace("\\", "/"), "sha256": sha256_file(onnx_path) if onnx_path and Path(onnx_path).exists() else None} if onnx_path else None,
        "created_at": datetime.now(UTC).isoformat(),
