...
export function getIntegrationMode(): IntegrationMode { return process.env.FAKE_NEWS_INTEGRATION_MODE === "live" ? "live" : "offline"; }
...
export async function getReport(reportName: string): Promise<Record<string, unknown>> {
  const safeName = reportName.replace(/[^a-zA-Z0-9_-]/g, "");
  if (getIntegrationMode() === "live") {
    try {
      const response = await fetchFastApi(`/reports/${safeName}`);
      if (response.ok) {
  ...
  try {
    ...
    const candidates = [
      path.resolve(process.cwd(), "reports", `${safeName}.json`),
      path.resolve(process.cwd(), "..", "reports", `${safeName}.json`),
      `/app/reports/${safeName}.json`,
    ];
