...
All application and server logging is configured through `src/config.py` with structlog’s context variables, a JSON `ProcessorFormatter`, and intercepted `uvicorn`, `uvicorn.error`, `uvicorn.access`, `fastapi`, and project logger hierarchies. Every request receives a bounded request ID from `X-Request-ID` or a generated UUID. Request start/completion/rejection events and drift enqueue events carry that identifier; request text, credentials, raw Redis URLs, and high-cardinality identifiers are never logged. Configuration is idempotent so reloads and test-created application instances do not accumulate duplicate JSON handlers.

The Redis rate limiter now has a closed/open/half-open circuit breaker. Consecutive connection or timeout failures are counted; after the configured threshold, the breaker opens and emits one structured CRITICAL transition alert. Open and half-open contention paths fail open with `(True, 0)` so inference traffic remains available while distributed limiting is degraded. After the recovery interval, one half-open probe is permitted; a successful Lua operation closes and resets the breaker, while failure reopens it. Breaker logs contain state, counts, thresholds, and exception types only, never credentials or connection URLs.

The Kubernetes base retains non-root execution, dropped capabilities, `allowPrivilegeEscalation: false`, `readOnlyRootFilesystem: true`, and `seccompProfile.type: RuntimeDefault` for API and Redis containers. `ingress.yaml` terminates TLS through the operator-provisioned `fake-news-api-tls` Secret and routes only to the API Service through the NGINX ingress class. `service-monitor.yaml` selects the API Service, scrapes `/metrics` on the named HTTP port, and uses the `release: prometheus` selector expected by the cluster Prometheus instance. Redis remains isolated by the existing NetworkPolicy.

The CI test gate runs `pytest -q --cov=src --cov-report=term-missing --cov-fail-under=95`. The repository’s `.coveragerc` documents the deliberately excluded CLI entrypoints (`src/train.py`, `src/evaluate.py`) and non-serving training/data-science adapters (`src/models/lstm.py`, `src/models/bert.py`, `src/features/embeddings.py`, `src/features/preprocessing.py`, `src/data/ingestion.py`, `src/evaluation/metrics.py`, `src/evaluation/plots.py`, `src/evaluation/search.py`, `src/models/classical.py`, `src/models/unsupervised.py`, and `src/tracking.py`). These modules remain covered by their existing functional/contract tests where available, but are outside the Day 5 SRE runtime gate because they require datasets, training backends, optional model downloads, plotting backends, or experiment servers. Ordinary production configuration, text normalization, serving, monitoring, export, and security logic remain in scope. Coverage is a blocking regression control rather than a substitute for functional, integration, security, or load testing.
