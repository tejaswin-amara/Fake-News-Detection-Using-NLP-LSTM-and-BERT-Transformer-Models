...
GitHub repository clone endpoint: [repository.git](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models.git). GitHub Actions workflow badges: [CI status](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/actions/workflows/ci.yml/badge.svg?branch=main), [CI workflow](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/actions/workflows/ci.yml), [CodeQL workflow](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/actions/workflows/codeql.yml), [CodeQL badge](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/actions/workflows/codeql.yml/badge.svg), and [Pages badge](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/actions/workflows/deploy-pages.yml/badge.svg). GitHub Pages production deployment: [Live showcase](https://tejaswin-amara.github.io/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/). Static technology and license badges: [Python 3.11](https://img.shields.io/badge/Python-3.11-3776AB) and [MIT license](https://img.shields.io/badge/License-MIT-1f6f54). **Use:** transparent visual status markers, live web presentation, and the documented clone command in `README.md`; the underlying CI policy remains the repository workflow. **Accessed:** 2026-08-21. **Terms:** externally served visual indicators only; implementation claims are supported by repository configuration and tests, not the badges.
...
GitHub Actions and upstream projects: [actions/checkout](https://github.com/actions/checkout), [actions/upload-artifact](https://github.com/actions/upload-artifact), [GitHub CodeQL SARIF upload action](https://github.com/github/codeql-action), [actions/labeler](https://github.com/actions/labeler), [Aqua Security Trivy Action](https://github.com/aquasecurity/trivy-action), [Python Semantic Release GitHub Actions documentation](https://python-semantic-release.readthedocs.io/en/latest/configuration/automatic-releases/github-actions.html), and [OpenSSF Scorecard action](https://github.com/ossf/scorecard-action). **Use:** the release, labeling, Scorecard, and immutable container-vulnerability workflows. Trivy is limited to OS/library vulnerability analysis with an explicit timeout because immutable Gitleaks scanning owns repository secret detection. **Accessed:** 2026-08-23. **Terms:** retain action-specific licenses and follow their current permission, pinning, and activation guidance. Semantic release remains disabled until the repository owner deliberately enables it with appropriate credentials/permissions; the labeler remains disabled until its labels and activation variable are owner-configured.
...
OpenSSF Scorecard [check documentation](https://github.com/ossf/scorecard/blob/main/docs/checks.md), the repository's prior [Scorecard workflow run](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/actions/runs/32565674353), the current [Scorecard workflow run](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/actions/runs/32641697857), and OSV advisory [PYSEC-2026-3552](https://osv.dev/PYSEC-2026-3552). **Use:** exact SARIF evidence, supply-chain control dispositions, and the unresolved cryptography compatibility constraint in `docs/security/code-scanning-remediation.md`. **Accessed:** 2026-08-23. **Terms:** workflow artifacts are repository evidence; Scorecard and OSV results are time-sensitive and must be re-run or re-checked before making a future remediation decision.
...
### SRC-054 — Gitleaks secret detection

[Gitleaks](https://github.com/gitleaks/gitleaks) documentation and the [Gitleaks Action v2 migration notes](https://github.com/gitleaks/gitleaks-action/blob/master/v2.md). **Use:** immutable, least-privilege secret detection, local configuration, incident handling, and regression policy for `.github/workflows/secret-scan.yml`, `.gitleaks.toml`, `docs/security/secret-handling.md`, contributor/agent instructions, and tests. **Accessed:** 2026-08-23. **Terms:** Gitleaks is MIT-licensed; Action licensing/ownership requirements must be reviewed when repository ownership changes. Findings are private security incidents requiring rotation and investigation; the repository does not use broad baselines, broad allowlists, or inline suppression for production content.

### SRC-055 — OWASP API, REST, and logging security guidance

The [OWASP API Security Project](https://owasp.org/www-project-api-security/), [REST Security Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html), and [Logging Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html). **Use:** FastAPI request/response boundaries, access-control and resource-consumption review, safe errors, request limits, CORS, security headers, structured logging, request identifiers, operational telemetry, and categorical exclusions for credentials and raw article text. **Accessed:** 2026-08-23. **Terms:** OWASP API Security project content is CC BY-SA 4.0; use this source as guidance with attribution and do not copy text beyond permitted quotation. **Files:** `src/serving/app.py`, tests, `docs/security/api-and-observability.md`, `docs/security/secret-handling.md`, architecture/runbooks, and adoption matrix.

### SRC-056 — Grafana k6 performance-test design

Grafana [k6 scenarios documentation](https://grafana.com/docs/k6/latest/using-k6/scenarios/), [thresholds documentation](https://grafana.com/docs/k6/latest/using-k6/thresholds/), and the [immutable setup action](https://github.com/grafana/setup-k6-action). **Use:** bounded metadata-only API performance smoke testing, explicit workload budgets, immutable setup, and pass/fail response/error thresholds in `tests/performance/`, `.github/workflows/performance-smoke.yml`, manual workflow documentation, and adoption matrix. **Accessed:** 2026-08-23. **Terms:** follow current Grafana k6 terms and package license; do not target production or external systems without owner approval, documented capacity, and consent.

### SRC-057 — Kubernetes production readiness, probes, and Secret handling

Kubernetes documentation for [liveness, readiness, and startup probes](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/), [Deployments](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/), and [good practices for Kubernetes Secrets](https://kubernetes.io/docs/concepts/security/secrets-good-practices/). **Use:** provider-neutral release gating, readiness/startup/liveness interpretation, controlled rollout and rollback design, and the rule that Secret manifests and values must not be committed. **Accessed:** 2026-08-24. **Terms:** use as operational guidance. Kubernetes documents that Secret values are base64-encoded rather than encrypted by default; operators must provide protected storage, least-privilege access, and suitable encryption/secret-management controls. **Files:** `docs/ADR/0007-production-deployment-boundary.md`, `docs/deployment/production-readiness.md`, `docs/deployment/production-rollout.md`, `scripts/verify_production_readiness.py`, and `tests/test_production_readiness.py`.
