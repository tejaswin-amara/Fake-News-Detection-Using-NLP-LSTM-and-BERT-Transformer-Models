...
This ledger records the repository's **evidence-based** disposition of supply-chain and code-scanning findings. It does not suppress, waive, or fabricate review evidence for any finding. The comparison uses the initial Scorecard SARIF evidence and the final SARIF published by the successful [OpenSSF Scorecard run][1] for main commit `9b634a3233b6f6b2e652319e178e8d0e8a202be7` on 2026-08-22. The final run contains **13** advisory entries, down from **30** in the initial SARIF: the Fuzzing finding was removed, PinnedDependencies advisories decreased from 23 to 7, and the Vulnerabilities score increased from 0 to 9. The remaining entries are described below exactly as reported by Scorecard.

| Evidence item | Initial state | Final state | Disposition |
|---|---:|---:|---|
| SARIF advisory entries | 30 | 13 | Measured reduction of 17 entries; no SARIF finding was filtered or dismissed. |
| `FuzzingID` | 1 | 0 | Resolved through a real ClusterFuzzLite Python/Atheris integration. |
| `PinnedDependenciesID` | 23 | 7 | Action and Python-base references were made immutable; seven pip-command hash advisories remain. |
...
The following pull requests were merged through protected main after their required CI, CodeQL, and container checks passed. The actions are traceable through GitHub pull requests #17–#21; their final result is independently captured in the final Scorecard run.[1]
...
The seven `PinnedDependenciesID` advisories identify `pip` commands in `Dockerfile`, `.clusterfuzzlite/build.sh`, `.github/workflows/ci.yml`, and `.github/workflows/fuzz.yml` that do not use a complete `--require-hashes` lock set. This is a **genuine residual supply-chain control**, not a false positive. The repository pins its direct dependencies and validates compatible resolution, but a platform-specific, fully hashed transitive lock must be generated and maintained before these advisories can be cleared. The required follow-up is to adopt a reproducible lock-generation process for each supported build platform and update every install command atomically; no hash checking has been disabled.[2]
...
|---|---|---|---|---|
| `PinnedDependenciesID` | Seven `pipCommand not pinned by hash` advisories | Direct pinning does not provide full transitive hash coverage. | Generate and maintain platform-appropriate hash locks; enforce them in all container and workflow installation commands. | Open engineering follow-up; no suppression. |
...
|---|---|---|---|
| `SASTID` | SAST ran on 20 of 25 commits. | CodeQL runs on current pull requests and main; historical commits cannot be retroactively changed. | Maintain the required CodeQL checks on every future change. |
| `CITestsID` | CI tested 14 of 17 merged pull requests. | Current protected-main CI is required; the score is historical. | Maintain required checks for every future pull request. |
| `CodeReviewID` | 0 of 14 approved changesets. | No review evidence was fabricated. | Configure a trusted second reviewer and require at least one approval for future changes. |
...
The final Scorecard run completed successfully. The final dependency remediation pull request also passed the repository's protected-main quality, CodeQL Python, CodeQL Actions, rootless container-build, and critical-vulnerability scan checks before merge. Local validation for that pull request included both requirement resolver checks, 139 regression tests, Ruff, source governance, and whitespace checks. `pip-audit` was run without suppression; it reported the remaining cryptography advisory and an unfixed transitive diskcache advisory, which are not treated as resolved by this document.
...
[1] Final [OpenSSF Scorecard workflow run](https://github.com/tejaswin-amara/Fake-News-Detection-Using-NLP-LSTM-and-BERT-Transformer-Models/actions/runs/32565674353), accessed 2026-08-22. Registered as SRC-052.
